...
import { Briefcase, TrendingUp, Layers, Cloud, Settings, Award, Check, ArrowRight } from 'lucide-react';
...
const SAPconsultpage = () => {
  ...
  const features = [
    { icon: Briefcase, title: "SAP S/4HANA", description: "Migration, implementation, and optimization of SAP's intelligent ERP.", color: "primary" },
    { icon: TrendingUp, title: "Business Transformation", description: "Process optimization and digital transformation consulting.", color: "secondary" },
    { icon: Layers, title: "SAP Modules Expertise", description: "FI/CO, MM, SD, PP, HR, and all other major SAP modules.", color: "primary" },
    { icon: Cloud, title: "SAP Cloud Solutions", description: "SuccessFactors, Ariba, Concur, and cloud-native applications.", color: "secondary" },
    { icon: Settings, title: "Custom Development", description: "ABAP, Fiori, and custom SAP application development.", color: "primary" },
    { icon: Award, title: "SAP Certified", description: "Certified consultants with proven enterprise implementation experience.", color: "secondary" }
  ];

  const solutions = ["SAP S/4HANA Migration", "ECC to S/4HANA Conversion", "SAP Fiori/UI5 Development", "SAP BW/4HANA & Analytics", "SAP SuccessFactors HCM", "SAP Ariba Procurement", "Custom ABAP Development", "SAP Integration Suite"];

  const process = [
    { step: "01", title: "Business Analysis", description: "Understand your processes and identify optimization opportunities." },
    { step: "02", title: "Solution Design", description: "Architect SAP solutions aligned with your business objectives." },
    { step: "03", title: "Implementation", description: "Deploy SAP systems with minimal business disruption." },
    { step: "04", title: "Support & Optimize", description: "Continuous improvement and dedicated support services." }
  ];
  ...
  return (
    <div className="bg-base-100 relative min-h-screen overflow-hidden">
      ...
      <div className="relative z-10">
        <PageHero
          badge="SAP Certified Partner"
          title="SAP Consulting Services"
          subtitle="Driving enterprise transformation and efficiency with expert SAP implementation, optimization, and support."
          buttonText="Request Consultation"
        />
        
        <Section title="Comprehensive SAP Expertise" subtitle="End-to-end SAP services for your digital transformation journey.">
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
            {features.map((feature, index) => <FeatureCard key={index} feature={feature} index={index} />)}
          </div>
        </Section>
        
        <Section title="Our Consulting Approach" subtitle="A strategic process designed for maximum impact and minimal disruption." isAlternateBg>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-x-8 gap-y-16">
            {process.map((item, index) => <ProcessCard key={index} item={item} index={index} />)}
          </div>
        </Section>

        <Section title="SAP Solutions We Deliver">
            <motion.div 
              initial={{ opacity: 0, y: 50 }}
