...
import { Server, Cloud, Shield, Zap, Database, Network, Check, ArrowRight } from 'lucide-react';
...
const VMwarepage = () => {
  ...
  const features = [
    { icon: Cloud, title: "vSphere Virtualization", description: "Enterprise-grade server virtualization and cloud infrastructure.", color: "primary" },
    { icon: Network, title: "NSX Networking", description: "Software-defined networking and security solutions.", color: "secondary" },
    { icon: Database, title: "vSAN Storage", description: "Hyper-converged infrastructure with integrated storage.", color: "primary" },
    { icon: Server, title: "vRealize Automation", description: "Multi-cloud automation and orchestration platform.", color: "secondary" },
    { icon: Shield, title: "Security & Compliance", description: "Advanced security controls and regulatory compliance.", color: "primary" },
    { icon: Zap, title: "Disaster Recovery", description: "Business continuity with VMware Site Recovery Manager.", color: "secondary" }
  ];

  const benefits = ["Reduce infrastructure costs by up to 50%", "Increase server utilization to 80%+", "24/7 monitoring and support services", "Seamless migration and deployment", "Multi-cloud management capabilities", "Enterprise-level security & compliance"];

  const process = [
    { step: "01", title: "Assessment & Planning", description: "We evaluate your infrastructure and design a tailored VMware solution." },
    { step: "02", title: "Architecture Design", description: "Create an optimized virtualization architecture for your needs." },
    { step: "03", title: "Implementation", description: "Seamless deployment with minimal downtime and disruption." },
    { step: "04", title: "Support & Optimization", description: "Ongoing monitoring, maintenance, and performance tuning." }
  ];
  ...
  return (
    <div className="bg-base-100 relative min-h-screen overflow-hidden">
      ...
      <div className="relative z-10">
        <PageHero
          badge="Virtualization Excellence"
          title="VMware Solutions for Modern Infrastructure"
          subtitle="Unlock efficiency, agility, and security with our expert VMware virtualization and cloud computing services."
          buttonText="Get Started"
        />
        
        <Section title="Comprehensive VMware Services" subtitle="Full-stack expertise across the VMware ecosystem and virtualization technologies.">
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
            {features.map((feature, index) => <FeatureCard key={index} feature={feature} index={index} />)}
          </div>
        </Section>
        
        <Section title="Our Implementation Process" subtitle="A meticulous approach to transforming your IT landscape." isAlternateBg>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-x-8 gap-y-16">
            {process.map((item, index) => <ProcessCard key={index} item={item} index={index} />)}
          </div>
        </Section>

        <Section title="Why Choose Our VMware Services">
            <motion.div 
              initial={{ opacity: 0, y: 50 }}
